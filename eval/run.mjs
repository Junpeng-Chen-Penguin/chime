...
function runCase(file) {
  ...
  const resolved = { ...spec, kb: spec.kb === true ? KB_SPEC : spec.kb }
...
function turnFacts(events, streamId) {
  ...
  const answer = texts.at(-1)?.text ?? ''
  return { status: done?.status, tools, answer, sources }
}
...
function structural(expect, f) {
  ...
  if (expect.searched === true && f.tools.length === 0) fails.push('应检索而未检索')
  if (expect.searched === false && f.tools.length > 0) fails.push('不应检索却检索了')
  if (f.tools.length > 3) fails.push(`检索 ${f.tools.length} 次超上限`)
  ...
  if (expect.citedValid) {
    const ns = [...f.answer.matchAll(/\[(\d+)\]/g)].map((m) => +m[1])
...
async function judge(criteria, f) {
  const material = (f.sources?.list ?? [])
    ...
    .join('\n\n')
  const prompt = [
    '你是评估员。根据检索资料判断 AI 回答是否满足判定标准，只输出 JSON：{"pass": true/false, "reason": "一句话"}',
    `判定标准：${criteria}`,
    `检索资料：\n${material || '（本轮无资料）'}`,
    `AI 回答：\n${f.answer}`
  ].join('\n\n')
