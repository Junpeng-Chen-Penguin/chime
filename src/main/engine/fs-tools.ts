...
什么时候用：
- 新建文件，或者已有文件要整个重写
- 修改已有文件的一部分不用它，用 edit_file 定点替换

用法：
- path 必须是绝对路径；可用的工作空间目录（绝对路径）在系统提示词的环境信息里
- 覆盖已有文件前必须先用 read_file 读过它（本会话内，且读后文件没被外部改过），否则这次调用会返回错误
- 写入前会弹出确认卡片等用户批准

返回语义：
- 成功返回「已新建」或「已更新」加文件路径与行数，据此向用户说明，不要另外复述内容
- 用户拒绝了这次写入时返回说明，文件没有改动：不要重发同一个调用，也不要说成已写入，先告诉用户这件事没做`

const EDIT_TOOL_DESCRIPTION = `对工作空间内已有文件做定点替换：把 old_string 换成 new_string。

什么时候用：
- 改已有文件的一部分。新建文件或整个重写用 write_file

用法：
- path 必须是绝对路径；编辑前必须先用 read_file 读过该文件（本会话内，且读后文件没被外部改过），否则这次调用会返回错误
- old_string 必须与文件原文精确一致（含缩进与换行；read_file 返回里的行号前缀不算内容），且在文件中唯一。取能唯一定位的最短一段，通常两到四行；不唯一时补足前后文，或传 replace_all 全部替换
- 编辑前会弹出确认卡片等用户批准

返回语义：
- 成功返回「已更新」加文件路径、替换处数与行数变化
- old_string 找不到或不唯一、文件不存在时返回错误说明，照说明改正后重调，不要虚构编辑结果
- 用户拒绝时返回说明，文件没有改动：不要重发，先告诉用户这件事没做`

const READ_TOOL_DESCRIPTION = `读取工作空间内的文本文件，返回带行号的内容。

什么时候用：
- 用户给了文件路径就直接读，不用先列目录核实；文件不存在会返回说明，不会出错中断
- 要看目录里有什么用 list_dir；要改文件先读再改

用法：
- path 必须是绝对路径；可用的工作空间目录（绝对路径）在系统提示词的环境信息里
- 默认从第 1 行读取最多 ${READ_LINES_DEFAULT} 行；未读完时结尾会提示剩余行数，用 offset 传起始行续读。已知只要某一段时只读那一段
- offset：起始行号（从 1 起）；limit：读取行数

返回语义：
- 内容按「行号:内容」逐行返回；空文件返回「文件为空」
- 文件不存在、目标是目录、不是文本文件时各返回一句说明，照说明处理，不要虚构文件内容`

const LIST_TOOL_DESCRIPTION = `列出工作空间内某个目录的内容：目录在前、各按字母排序，目录名以 / 结尾，不含隐藏文件。

什么时候用：
- 不知道要找的文件叫什么、在哪一层时，先列目录再决定读哪个；已知路径的文件直接用 read_file 读
- 找目录里有没有某个文件，列一次目录看，不要逐个猜路径去读

用法：
- path 必须是绝对路径；可用的工作空间目录（绝对路径）在系统提示词的环境信息里
- depth：递归层数（默认 1 只列一层，最多 ${LIST_DEPTH_MAX}）；超过 ${LIST_LIMIT} 条会截断并提示，这时把 path 指到更深一层再列

返回语义：
- 空目录返回「目录为空」；目录不存在或目标是文件时返回一句说明，照说明处理`
